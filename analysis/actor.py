...
for j in range(0,len(stockList)):
    ...
    closing = dis.values.tolist()
    close = []
    for item in closing:
        close.append(float(item[0]))
    try:
        initial_money = 10000
        window_size = 30
        skip = 1
        ...
        agent = Agent(state_size = window_size, 
                      ...
                      trend = close, 
                      skip = skip)
        agent.train(iterations = epochs, checkpoint = 10, initial_money = initial_money)
        ...
        plt.title('total gains %f, total investment %f%%'%(total_gains, invest)+ ' SYM : '+ stockList[j])
        plt.legend()
        plt.show()
           
    except:
        print("Failed to predict for " + stockList[j])
